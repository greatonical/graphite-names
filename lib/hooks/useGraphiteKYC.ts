    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_GRAPHITE_API_KEY) {
        apiUrl.searchParams.set(
          "apikey",
          process.env.NEXT_PUBLIC_GRAPHITE_API_KEY
        );
      }
      ...
      try {
        ...
        console.log(
          "⚠️ GraphiteKYC - Direct API call failed, trying proxy...",
          directError
        );
        ...
        try {
          ...
          if (response.ok) {
            ...
            throw new Error(
              `Proxy API responded with status: ${response.status}`
            );
          }
        } catch (proxyError) {
      ...
      if (data.status === "1" && data.result) {
        ...
        kycData = {
          ...
          reputation: parseInt(
            result.reputation || result.reputationScore || "0"
          ),
          status: data.status,
  ...
  const fetchUserKYC = useCallback(async () => {
    ...
    try {
      ...
      setState((prev) => ({
        ...
        loading: false,
        error:
          "Unable to fetch KYC data from Graphite API. API may be unavailable or require authentication.",
        data: {
          address: address,
          kycLevel: 0,
          isActivated: false,
